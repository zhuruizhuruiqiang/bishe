...
"""
加载图像数据的函数,dataset_path即图像CIFAR-100的路径
加载数据后，划分为train_data,valid_data,test_data三个数据集
...
def load_data(dataset_path):
    ...
    train_set_x, train_set_y, train_set_z= shared_dataset(train_data, train_label,tra_label)
    valid_set_x, valid_set_y ,valid_set_z= shared_dataset(valid_data, valid_label,val_label)
    test_set_x, test_set_y ,test_set_z   = shared_dataset(test_data, test_label,te_label)
...
def evaluate_pictures(learning_rate=0.03, n_epochs=10000,
                      dataset='/Users/jimubox/Downloads/bishe/cifar-100-python',
                      nkerns=[10, 50], batch_size=10):
    # 随机数生成器，用于初始化参数
    ...
    # (3,32, 32) is the size of  images.
    layer0_input = x.reshape((batch_size, 3, 32, 32))

    # 第一个卷积+maxpool层
    # 卷积后得到：(32-3+1 , 32-3+1) = (30, 30)
    # maxpooling后得到： (30/2, 30/2) = (15, 15)，因为忽略了边界
    ...
    # 第二个卷积+maxpool层,输入是上层的输出，即(batch_size, nkerns[0], 15, 15)
    # 卷积后得到：(15-2+1 , 15-2+1) = (14, 14)
    # maxpooling后得到： (14/2, 14/2) = (7, 7)，因为忽略了边界
    ...
    # 因此将上一层的输出(batch_size, nkerns[1], 5, 5)转化为(batch_size, nkerns[1] * 5* 5),用flatten
    layer2_input = layer1.output.flatten(2)
    layer2 = HiddenLayer(
        rng,
        input=layer2_input,
        n_in=nkerns[1] * 7 * 7,
        n_out=1000,  # 全连接层输出神经元的个数，自己定义的，可以根据需要调节
        activation=T.tanh
    )

    # 分类器
    layer3 = LogisticRegression(input=layer2.output, n_in=1000, n_out=100)  # n_in等于全连接层的输出，n_out等于40个类别
    ...
    # 中，batch_size=1000,n_train_batches=100000/1000=100, paticence可以相应地设置为10000，这个可以根据实际情况调节，调大一点也无所谓
    patience = 100000
    patience_increase = 2
