...
def load_params(params_file):
    ...
    layer0_params = pickle.load(f, encoding='ISO-8859-1')
    layer1_params = pickle.load(f, encoding='ISO-8859-1')
    layer2_params = pickle.load(f, encoding='ISO-8859-1')
    layer3_params = pickle.load(f, encoding='ISO-8859-1')
    f.close()
...
def load_data(dataset_path):
    ...
    dic_test = pickle.load(fo, encoding='ISO-8859-1')
    fo.close()
    test_data = dic_test['data'][5000:10000]
    test_label = dic_test['labels'][5000:10000]
...
def use_CNN(dataset='/Users/jimubox/Downloads/bishe/cifar-10-batches-py', params_file='params.pkl', nkerns=[40, 20]):
    # 读取测试的图像，这里读取整个olivettifaces.gif，即全部样本，得到faces、label
    test, label = load_data(dataset)
    test_num = test.shape[0]  # 有多少张图

    # 读入参数
    layer0_params, layer1_params, layer2_params, layer3_params = load_params(params_file)

    x = T.matrix('x')  # 用变量x表示输入的图像数据，作为layer0的输入

    ######################
    # 用读进来的参数初始化各层参数W、b
    ######################
    layer0_input = x.reshape((test_num, 3, 32, 32))
    layer0 = LeNetConvPoolLayer(
        input=layer0_input,
        params_W=layer0_params[0],
        params_b=layer0_params[1],
        image_shape=(test_num, 3, 32, 32),
    ...
    layer1 = LeNetConvPoolLayer(
        ...
        image_shape=(test_num, nkerns[0], 15, 15),
    ...
    layer3 = LogisticRegression(input=layer2.output, params_W=layer3_params[0], params_b=layer3_params[1], n_in=100,
                                n_out=10)
